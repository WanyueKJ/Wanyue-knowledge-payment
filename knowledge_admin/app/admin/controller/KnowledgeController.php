...
// | Created by Wanyue
// +----------------------------------------------------------------------
// | Copyright (c) 2017~2020 http://www.sdwanyue.com All rights reserved.
// +----------------------------------------------------------------------
...
class KnowledgeController extends AdminBaseController
{
    ...
    function setstatus()
    {
        ...
        $rs = CourseModel::where(['id' => $id])->update($update);
        if (!$rs) {
            $this->error("操作失败！");
        }
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            //知识付费分类
            $classid = $data['knowledgeid'];
            if ($classid < 1) {
                $this->error('请选择知识付费分类');
            }
            ...
            $this->success("添加成功！", url("knowledge/index", ['sort' => $sort]));
        }
    }
    ...
    {
        $courseid = $this->request->param('id', 0, 'intval');
        $is_hot = $this->request->param('is_hot', 0, 'intval');

        if (!$courseid || $is_hot < 0) {
            $this->error('参数错误');
        }

        $count = db('course')->where('is_hot', 1)->where('status', 1)->count();
        if ($is_hot == 1 && $count == 3) {
            $this->error('热门精选最多设置3个');
        }

        $status = db('course')->where('id', $courseid)->value('status');
        if ($status != 1 && $is_hot == 1) {
            $this->error('该课程未上架, 请先上架');
        }

        $rs = db('course')->where('id', $courseid)->update(['is_hot' => $is_hot]);
        if (!$rs) {
            $this->error('设置失败');
        }
