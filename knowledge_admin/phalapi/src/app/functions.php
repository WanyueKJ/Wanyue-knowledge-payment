...
//		$str = '1ecxXyLRB.COdrAi:q09Z62ash-QGn8VFNIlb=fM/D74WjS_EUzYuw?HmTPvkJ3otK5gp';
   ...
   *  body     提示标题
   */
    function smallPay($orderid,$money,$url,$body='充值虚拟币', $openid) {
        ...
        //配置参数检测
        if($configpri['know_small_appid']== "" || $configpri['know_small_mchid']== "" || $configpri['know_small_key']== ""){
            $rs['code'] = 1003;
            $rs['msg'] = '小程序未配置';
            return $rs;
        }
        ...
        $paramarr = array(
            "appid"       =>   $configpri['know_small_appid'],
            "body"        =>    $body,
            "mch_id"      =>    $configpri['know_small_mchid'],
            "nonce_str"   =>    $noceStr,
            "notify_url"  =>    $url,
            "out_trade_no"=>    $orderid,
            "total_fee"   =>    $money*100,
        ...
        ksort($paramarr);
        $sign = sign($paramarr,$configpri['know_small_key']);//生成签名
        $paramarr['sign'] = $sign;
        $paramXml = "<xml>";
        foreach($paramarr as $k => $v){
            $paramXml .= "<" . $k . ">" . $v . "</" . $k . ">";
        ...
        if(curl_errno($ch)){
            //print curl_error($ch);
            file_put_contents(API_ROOT.'/../data/log/appsmallpay_'.date('Y-m-d').'.txt',date('y-m-d H:i:s').' 提交参数信息 ch:'.json_encode(curl_error($ch))."\r\n",FILE_APPEND);
        }
        curl_close($ch);
        ...
        ksort($paramarr2);
        $paramarr2["sign"] = sign($paramarr2,$configpri['know_small_key']);//生成签名
    ...
    *  body     提示标题
    */
    function hfivePay($orderid,$money,$url,$body='充值虚拟币', $openid) {
        ...
        $paramarr = array(
            ...
            "trade_type"  =>    "MWEB",
            'openid'     =>     $openid,
            'spbill_create_ip' => getClientIp(), //用户终端真实IP地址
        );

        ksort($paramarr);
        $sign = sign($paramarr,$configpri['pc_wx_key']);//生成签名
        $paramarr['sign'] = $sign;
        $paramXml = "<xml>";
        foreach($paramarr as $k => $v){
            $paramXml .= "<" . $k . ">" . $v . "</" . $k . ">";
        ...
        if(curl_errno($ch)){
            //print curl_error($ch);
            file_put_contents(API_ROOT.'/../data/log/h5pay_'.date('Y-m-d').'.txt',date('y-m-d H:i:s').' 提交参数信息 ch:'.json_encode(curl_error($ch))."\r\n",FILE_APPEND);
        }
        curl_close($ch);
        ...
        ksort($paramarr2);
        $paramarr2["sign"] = sign($paramarr2,$configpri['pc_wx_key']);//生成签名
