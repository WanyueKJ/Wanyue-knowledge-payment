<?php /*a:2:{s:85:"D:\My_project\Wanyue-knowledge-payment-admin/themes/admin_htcyltd/admin\user\add.html";i:1608172448;s:84:"D:\My_project\Wanyue-knowledge-payment-admin/themes/admin_htcyltd/public\header.html";i:1609814284;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
<form method="post" class="layui-form js-ajax-form" action="<?php echo url('user/addpost'); ?>" style="padding: 20px 30px 0 0;">
	...
	<div class="layui-form-item">
		...
		<div class="layui-input-block">
			<input type="text" class="layui-input" required lay-verify="required" id="input-user_pass" name="user_pass" placeholder="******">
		</div>
	</div>
	<div class="layui-form-item">
		<label for="input-user_email" class="layui-form-label"><span class="form-required">*</span><?php echo lang('EMAIL'); ?></label>
		<div class="layui-input-block">
			<input type="text" class="layui-input" required lay-verify="required" id="input-user_email" name="user_email">
		</div>
	</div>
	<div class="layui-form-item">
		...
		<div class="layui-input-block">
			<?php if(is_array($roles) || $roles instanceof \think\Collection || $roles instanceof \think\Paginator): if( count($roles)==0 ) : echo "" ;else: foreach($roles as $key=>$vo): ?>
				<input value="<?php echo $vo['id']; ?>" type="checkbox" name="role_id[]" title="<?php echo $vo['name']; ?>"<?php if(cmf_get_current_admin_id() != 1 && $vo['id'] == 1): ?>disabled="true"<?php endif; ?>>
