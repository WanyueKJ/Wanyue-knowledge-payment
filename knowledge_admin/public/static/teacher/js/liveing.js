...
function liveCountdown(){
    ...
    timer=window.setInterval(function() {
        if (time_t > 0) {
            ...
            timer = null;
            $('.liveing_c_c_t_time').hide();
            $('.liveing_c_c_t_tips').html('授课中');
        }
    },1e3);
}
...
var chat={
    ...
    init:function(){
        ...
        _this.js_audio.on('click','.audio_send',function(){
            ...
            recorder.getBlob(function(blob){
                ...
                $.ajax({
                    type: 'post',
                    url: '/teacher/liveing/addAudio', //上传文件的请求路径必须是绝对路劲
                    data: formData,
                    dataType:'json',
                    cache: false,
                    processData: false,
    ...
    getMsg:function(type=0){
        ...
        $.ajax({
            url:'/teacher/liveing/getChat',
            type:'POST',
            data:{courseid:courseid,lessonid:lessonid,type:type,lastid:lastid},
            dataType:'json',
            error:function(e){
                if(type==0){
                    _this.isscroll_0=1;
...
var ppt={
    ...
    changePPT:function(activeIndex){
        if(_DATA.roominfo.user_type==1){
            $.ajax({
                url:'/teacher/liveing/upPPTindex',
                type:'POST',
...
async function joinAgora(){
    ...
    if(_DATA.teacherinfo.id == _DATA.userinfo.id){
        let role='host';
...
var Live={
            ...
            success:function(data){
                ...
                layer.msg('授课结束',{},function(){
                    if(lessonid>0){
                        ...
                        location.href='/teacher/live/index';
                    }
                });
            },
            error:function(e){
    ...
    setScreen:async function(){
        ...
        _this.upScreen(1);
        rtc.client_screen = AgoraRTC.createClient({mode: 'live', codec: "h264"});

        let role='host';
        await rtc.client_screen.setClientRole(role);

        await rtc.client_screen.join(option.appID,  option.channel, option.token, parseInt(option.channel_screen));
        ...
        AgoraRTC.createScreenVideoTrack({
            ...
            screen_Track.play('screen',{fit: "contain"});
            rtc.client_screen.publish(screen_Track);

            rtc.published_screen=true;
            screen_Track.on("track-ended", () => {
                rtc.published_screen=false;
                if(_DATA.roominfo.user_type==1){
    ...
    stopScreen:async function(){
        ...
        if(rtc.client_screen){
            ...
            await rtc.client_screen.unpublish();
            await rtc.client_screen.leave();

            rtc.client_screen=null;
            screen_Track=null;
        }
        _this.quitScreen();
        console.log('屏幕共享停止了');
...
var Student={
    ...
    getNums:function(){
        var _this=this;
        $.ajax({
            url:'/teacher/liveing/getUserListNum',
            type:'POST',
            data:{stream:stream},
            dataType:'json',
            success:function(data){
